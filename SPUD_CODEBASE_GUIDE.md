...
The Spud codebase is not as intimidating as it seems!

Most of Spud's AI code lives in the folder `src/vs/workbench/contrib/void/`. The folder is called `void/` for merge hygiene with the upstream Void repo — the **product** is Spud.

The purpose of this document is to explain how Spud's codebase works. If you want build instructions instead, see [Contributing](./HOW_TO_CONTRIBUTE.md).
...
### VS Code rundown
Here's a VS Code rundown if you're just getting started with Spud. You can also see Microsoft's [wiki](https://github.com/microsoft/vscode/wiki/Source-Code-Organization) for some pictures. VSCode is an Electron app. Electron runs two processes: a **main** process (for internals) and a **browser** process (browser means HTML in general, not just "web browser").
<p align="center" >
...
Here's a picture of all the dependencies that are relevant between the time you first send a message through Spud's sidebar, and the time a request is sent to your provider.
...
Spud has two types of Apply: **Fast Apply** (uses Search/Replace, see below), and **Slow Apply** (rewrites whole file).
...
>>>>>>> UPDATED
```
This is what allows Spud to quickly apply code even on 1000-line files. It's the same as asking the LLM to press Ctrl+F and enter in a search/replace query.
...
When Spud wants to change your code, it just writes to a text model. This means all you need to know to write to a file is its URI - you don't have to load it, save it, etc. There are some annoying background URI/model things to think about to get this to work, but we handled them all in `voidModelService`.

### Settings Inner Workings
We have a service `voidSettingsService` (name kept for upstream merge hygiene) that stores all your Spud settings — providers, models, global settings. Think of it as an implicit dependency for any of the core services:
...
### Build process
If you want to know how our build pipeline works, see the Spud fork at `spud/spud-builder` (cloned from [void-builder](https://github.com/voideditor/void-builder)).



## VS Code Codebase Guide

For additional references, here is a curated list of links to get up and running with VS Code (originally assembled by the upstream Void team).
<details>
...
Spud is not an extension (it's a full IDE fork), so these links are not required, but they are useful if you ever need to reason about the extension API.
