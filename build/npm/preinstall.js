...
if (!process.env['VSCODE_SKIP_NODE_VERSION_CHECK']) {
	...
	if (majorNodeVersion >= 23) {
		console.error('\x1b[1;31m*** Node.js v23+ is not supported for this workspace: native addons (e.g. @vscode/spdlog) fail to build with current node-gyp. Use Node.js 20.x LTS (see .nvmrc), then run bun install again. Example: nvm install && nvm use\x1b[0;0m');
		throw new Error();
	}
}
...
function usingBun() {
	...
	return execpath.includes('bun');
}

if (process.env['npm_execpath']?.includes('yarn')) {
	console.error('\x1b[1;31m*** Seems like you are using `yarn` which is not supported in this repo any more, please use `npm i` or `bun install` instead. ***\x1b[0;0m');
	throw new Error();
}
...
function installHeaders() {
	const gypCwd = path.join(__dirname, 'gyp');
	if (usingBun()) {
		const isCi = process.env['npm_command'] === 'ci';
		const args = isCi ? ['install', '--frozen-lockfile'] : ['install'];
		const result = cp.spawnSync(process.platform === 'win32' ? 'bun.exe' : 'bun', args, {
			env: process.env,
			cwd: gypCwd,
			stdio: 'inherit',
			shell: true
