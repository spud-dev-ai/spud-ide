...
function getPackageManagerBinary() {
	if (usingBun()) {
...
function npmInstall(dir, opts) {
	...
	if (process.env['VSCODE_REMOTE_DEPENDENCIES_CONTAINER_NAME'] && /^(.build\/distro\/npm\/)?remote$/.test(dir)) {
		...
		// Avoid Node DEP0190: do not pass argv with shell:true (unsafe concatenation).
		const installOpts = { ...opts, shell: process.platform === 'win32' };
		run(getPackageManagerBinary(), getInstallArgs(), installOpts);
	}
...
try {
	require('./ensureRollupDarwinX64').main();
} catch (e) {
	console.warn('[postinstall] ensureRollupDarwinX64:', e && e.message ? e.message : e);
}
