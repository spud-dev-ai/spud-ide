...
const tasks = compilations.map(function (tsconfigFile) {
	...
	// Split so `compile-extensions` can run all cleans before all compiles (avoids rimraf vs gulp.dest races → chmod ENOENT on `out`).
	const compileExtensionOnlyTask = task.define(`compile-extension-only:${name}`, () => {
	...
	const compileTask = task.define(`compile-extension:${name}`, task.series(cleanTask, compileExtensionOnlyTask));
	...
	gulp.task(compileExtensionOnlyTask);
	gulp.task(compileTask);
	gulp.task(watchTask);

	return { transpileTask, compileTask, watchTask, compileBuildTask, cleanTask, compileExtensionOnlyTask };
});
...
// Run each extension compile one-after-another after parallel clean. Parallel compile-only still races
// vinyl-fs mkdir/chmod on nested dirs (e.g. vscode-api-tests/out/singlefolder-tests) on some Node/fs combos.
const compileExtensionsTask = task.define(
	'compile-extensions',
	task.series(
		task.parallel(...tasks.map(t => t.cleanTask)),
		...tasks.map(t => t.compileExtensionOnlyTask)
	)
);
