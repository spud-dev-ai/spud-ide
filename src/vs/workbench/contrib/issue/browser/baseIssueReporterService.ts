...
export class BaseIssueReporterService extends Disposable {
	...
	public searchVSCodeIssues(title: string, issueDescription?: string): void {
		if (title) {
			this.searchGitHub('spud-dev-ai/spud-ide', title);
		} else {
			this.clearSearchResults();
		}
	}
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('vscode', "Spud"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "A Spud extension"), false));
	...
	public async renderBlocks(): Promise<void> {
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "This extension handles issues outside of Spud"));
