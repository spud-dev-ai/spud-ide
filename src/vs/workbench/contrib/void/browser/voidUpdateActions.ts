...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Spud, please download the latest version! [Spud](https://spud.dev/download)!'
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://spud.dev/download')
				}
			})
		}
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Spud: There was an error checking for updates. If this persists, please get in touch or reinstall from [spud.dev](https://spud.dev/download).`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	metricsService.capture(`Spud Update ${metricsTag}: Checking...`, {})
	const res = await voidUpdateService.check(explicit)
	if (!res) {
		const notifController = notifyErrChecking(notifService);
		metricsService.capture(`Spud Update ${metricsTag}: Error`, { res })
		return notifController
	}
	else {
		if (res.message) {
			const notifController = notifyUpdate(res, notifService, updateService)
			metricsService.capture(`Spud Update ${metricsTag}: Yes`, { res })
			return notifController
		}
		else {
			metricsService.capture(`Spud Update ${metricsTag}: No`, { res })
			return null
		}
	}
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'Spud: Check for Updates'),
		});
	}
