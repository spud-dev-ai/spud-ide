...
export const VoidInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] ${multiline ? 'overflow-y-auto' : 'overflow-x-hidden overflow-y-hidden'} text-void-fg-1 placeholder:text-void-fg-3 ${className}`}
			style={{
...
export const VoidButtonBgDarken = ({ children, disabled, onClick, className, primary }: { children: React.ReactNode; disabled?: boolean; onClick: () => void; className?: string; primary?: boolean }) => {
	return <button
		type="button"
		disabled={disabled}
		className={`vc-settings-btn${primary ? ' vc-settings-btn--primary' : ''}${className ? ` ${className}` : ''}`}
		onClick={onClick}
	>{children}</button>
}
