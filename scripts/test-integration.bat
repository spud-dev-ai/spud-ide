...
echo.
echo ### Colorize tests
call bun run test-extension -- -l vscode-colorize-tests
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### Terminal Suggest tests
call bun run test-extension -- -l terminal-suggest --enable-proposed-api=vscode.vscode-api-tests
if %errorlevel% neq 0 exit /b %errorlevel%
...
echo.
echo ### Markdown tests
call bun run test-extension -- -l markdown-language-features
if %errorlevel% neq 0 exit /b %errorlevel%
...
echo.
echo ### Ipynb tests
call bun run test-extension -- -l ipynb
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### Notebook Output tests
call bun run test-extension -- -l notebook-renderers
if %errorlevel% neq 0 exit /b %errorlevel%
...
mkdir %CFWORKSPACE%
call bun run test-extension -- -l configuration-editing
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### GitHub Authentication tests
call bun run test-extension -- -l github-authentication
if %errorlevel% neq 0 exit /b %errorlevel%
